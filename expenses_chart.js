...
let expenseChart = new Chart(ctx, {             // Need uppercase C in Chart()
    type: 'doughnut',
    data: {
        datasets: [
            {
...
function addExpenseToChart(name, amount) {
    ...
    let color = chartColors[ colorCount % chartColors.length ]              // % modular operation

    console.log(colorCount, color)                                  // Use in Dev. tools to show colors are cycling
